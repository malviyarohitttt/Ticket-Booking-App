...
export class UsersService {
  ...
  async validateCredentials(
    ...
    if (userMeta.passwordHash === passwordHash) {
      return {
        id: user.id,
        type:
          user.role === UserRoles.Manager ? UserType.Manager : UserType.User,
      };
    }
  ...
  async setRole(userId: number, role: UserRoles): Promise<User> {
    await this.cacheManager.del(
      getAccessGuardCacheKey({
        id: userId,
        type: role === UserRoles.Manager ? UserType.Manager : UserType.User,
      }),
    );
    return await this.prisma.user.update({
      data: { role },
  ...
  async myEvents(ctx: AuthenticatedUser) {
